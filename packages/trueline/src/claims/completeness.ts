...
const everyFileBelongsToAZone: Claim = {
  ...
  guidance:
    "A file matches no zone, so no boundary or seam rule applies to it. Move it under an existing zone, or declare a zone that covers it. Run `{trueline} explain <file>` to see what a location would allow.",
  check: ({ root, zones }) =>
