...
const reportWith = (...except: SiblingException[]): Report =>
  check({
    root: LAYERED,
...
const layered = (...except: SiblingException[]): string[] => messagesIn(reportWith(...except), CLAIM);

describe("an order among the directories a group shares", () => {
  it("lets a shared directory reach only the shared siblings its list allows", () => {
    expect(layered("_ui", "_state", { shared: "_root", allow: ["_state"] })).toEqual([ROOT_REFUSED]);
  });

  it("says so when a directory was given no reach at all", () => {
    expect(layered("_ui", "_root", { shared: "_state", allow: [] })).toEqual([STATE_REFUSED]);
  });

  it("keeps a bare name reaching every shared sibling, as it always has", () => {
    expect(layered("_ui", "_state", "_root")).toEqual([]);
  });

  it("still lets an island reach every shared directory, whatever the order among them", () => {
    const said = layered("_ui", { shared: "_state", allow: [] }, { shared: "_root", allow: [] });

    expect(said.join()).not.toContain("is a");
  });

  it("reads the allowed reach as patterns, like the names beside it", () => {
    const said = layered("_ui", "_state", { shared: "_root", allow: ["_st*"] });
  ...
  it("tells the reader that widening the reach is not the fix", () => {
    const claim = claimIn(reportWith(), CLAIM);
