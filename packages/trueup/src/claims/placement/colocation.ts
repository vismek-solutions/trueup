...
const PLACEMENT =
  "A zone exports a value that only one other zone uses, so the seam it crosses carries nothing a second caller needs: a symbol in a shared package that one consumer uses is not shared, it is that consumer's code in the wrong place. Which thing to move is already decided for you by the shape of the finding, so do not work it out again from the file. A finding that counts the exports means nothing outside one consumer reads that file at all, its own zone included: move the file into that consumer, and all of them close at once. A finding that names one declaration means the rest of the file has other readers, so the file stays. Before moving that one declaration, ask whether the seam should carry it at all: a value the caller derives from an argument it hands the same collaborator belongs to that collaborator, which can derive it itself and leave the two nothing to disagree about. Moving it is the fix only when it does not. A second consumer arriving later is a reason to move it back then, not a reason to leave it now. Type-only edges are not reported, because a type can be used through a value without ever being imported. Giving a zone a role to make this finding go away is the fix that defeats the check: a role is honest only for a zone that never owns what it uses.";

const INTERNALS =
  "A test reaches a symbol that nothing outside its own directory calls, so the test knows a decomposition none of the callers know. Fold that symbol into the neighbour that uses it and the behaviour is unchanged while the test breaks, which is what it means for a test to be bound to an implementation detail rather than to behaviour. Reach the behaviour through the surface the production callers already go through, and the split underneath is free to move. When that is genuinely too expensive, because a handful of cases each need their own fixture to drive from outside, the symbol is asking to become a module with a caller of its own, not a wider surface on the one it sits in. A zone with the wiring role is not reported, because a composition root has no internals to protect. Something the package publishes belongs in a zone with the api role, and is surface wherever it is declared. Widening that surface so the direct test becomes legitimate, or adding a production caller to justify it, both leave the codebase worse than the finding did.";
