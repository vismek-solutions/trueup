...
const GUIDANCE =
  "The exports of this file fall into groups that no reader takes from across, so one file is serving audiences that never meet. Split it, and each half sits with the readers it has. When a group is a single export, dissolving it usually beats rehousing it: a value one caller derives from what it already holds belongs inside that caller, and then there is no second file to place — reaching for a new home first is the common mistake here. A declaration that names another in the same file counts with it, since the two cannot be moved apart. An export nothing reads is left out, because unused code is a different finding. A zone with a role is not reported and does not count as a reader: a barrel, a composition root and a test suite each answer to readers this analysis does not own.";
